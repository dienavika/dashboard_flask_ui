...
.sort_values(by='Reviews', ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    ...
    category_type = df2.groupby('Type')['Type'].count()
    fig = plt.figure(figsize=(5,5))
    def func(pct, allvalues):
        absolute = int(round(pct / 100.*sum(allvalues)))
        return f'{absolute:,} ({pct:.1f}%)'
    plt.pie(category_type, labels=category_type.index, autopct=lambda pct: func(pct, category_type), startangle=140, colors=plt.cm.Paired(range(len(category_type))))
